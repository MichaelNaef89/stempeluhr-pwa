...
| Bereich | Details |
|---|---|
| **+ Eintrag** | Einzige Hauptaktion auf dem Stempeln-Screen (grosser Button) **und** in der Tagesansicht für jeden beliebigen Tag (z.B. über Monat → Tag antippen): Tätigkeit wählen, dann **Stunden** (Stepper ±0.25h, Direkteingabe, Presets 1/2/4/8h) oder **Zeit** (Von/Bis-Picker mit Live-Dauer-Anzeige) – erzeugt in einem Schritt ein komplettes Kommt/Geht-Paar, max. 4 Paare pro Tag. Schliesst nahtlos an den letzten Eintrag des jeweiligen Tages an (kein Überschneiden möglich, wird validiert). Ohne bisherigen Eintrag rechnet der Stunden-Modus von „jetzt" rückwärts (nur für „heute" sinnvoll, sonst deaktiviert) – der Zeit-Modus braucht dagegen nie einen Anker und funktioniert immer, auch als allererster Eintrag eines fremden Tages. |
| **+ nachtragen** | Ergänzt gezielt einen einzelnen fehlenden Stempel (nur Kommt oder nur Geht) mit fester Uhrzeit – z.B. um eine Alt-Session zu schliessen oder eine einzelne Lücke zu füllen, ohne gleich ein ganzes Paar über „+ Eintrag" anzulegen. |
